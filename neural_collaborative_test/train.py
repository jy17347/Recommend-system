...
model.compile(optimizer='adam', loss=loss, metrics=[tf.keras.metrics.Precision(),tf.keras.metrics.Recall(),tf.keras.metrics.AUC()])
history = model.fit([x_train, y_train], label_train, epochs=50, batch_size = 300,validation_data=([x_val, y_val], label_val))
